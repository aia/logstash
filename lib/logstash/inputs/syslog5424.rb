...
class LogStash::Inputs::Syslog5424 < LogStash::Inputs::Base
  config_name "syslog5424"
  ...
  def register
    @grok_filter = LogStash::Filters::Grok.new({
      "type"    => [@config["type"]],
      "pattern" => ["<%{POSINT:priority}>%{SYSLOG5424}"],
    })
  ...
  def run(output_queue)
    # udp server
    Thread.new do
      LogStash::Util::set_thread_name("input|syslog5424|udp")
      begin
    ...
    # tcp server
    Thread.new do
      LogStash::Util::set_thread_name("input|syslog5424|tcp")
      begin
  ...
  def tcp_listener(output_queue)
    @logger.info("Starting syslog5424 tcp listener", :address => "#{@host}:#{@port}")
    ...
    loop do
      ...
      Thread.new(client) do |client|
        ...
        LogStash::Util::set_thread_name("input|syslog5424|tcp|#{ip}:#{port}}")
        if ip.include?(":") # ipv6
          source = "syslog5424://[#{ip}]/"
        else
          source = "syslog5424://#{ip}/"
        end
  ...
  def syslog_relay(event, url)
    ...
    if !event.tags.include?("_grokparsefailure")
      ...
      url = "syslog5424://#{Socket.gethostname}/" if url == "syslog5424://127.0.0.1/"
...
end # class LogStash::Inputs::Syslog5424
